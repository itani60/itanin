...
class SmartphoneInfo {
    ...
    createPriceChart(priceHistory, period = 30) {
        ...
        const priceRange = Math.max(1, maxPrice - minPrice); // avoid divide-by-zero

        // Thin out points for smaller screens/long ranges
        ...
        // Scales
        const x = (i) => margin.left + (i / Math.max(1, data.length - 1)) * W;
        const y = (p) => margin.top + (1 - (p - minPrice) / priceRange) * H;
        ...
        stop1.setAttribute('offset', '0%'); stop1.setAttribute('stop-color', '#28a745');
        const stop2 = document.createElementNS(svgNS, 'stop');
        stop2.setAttribute('offset', '50%'); stop2.setAttribute('stop-color', '#ffc107');
        const stop3 = document.createElementNS(svgNS, 'stop');
        stop3.setAttribute('offset', '100%'); stop3.setAttribute('stop-color', '#dc3545');
        ...
        for (let i = 0; i <= gridCount; i++) {
            const gy = margin.top + (i / gridCount) * H;
            ...
            // Price labels at grid
            const priceAtLine = maxPrice - (i / gridCount) * priceRange;
            const t = document.createElementNS(svgNS, 'text');
            t.setAttribute('x', margin.left - 10);
            t.setAttribute('y', gy + 4);
            t.setAttribute('text-anchor', 'end');
            t.setAttribute('class', 'y-label');
            t.textContent = this.formatPrice(Math.round(priceAtLine));
            svg.appendChild(t);
        }
        ...
        const labelStep = Math.max(1, Math.floor(data.length / (isMobile ? 4 : 8)));
        data.forEach((d, i) => {
            const cx = x(i);
            const cy = y(d.price);
            ...
            c.setAttribute('aria-label', `${d.formattedDate}: ${this.formatPrice(d.price)}`);

            // Native tooltip
            const title = document.createElementNS(svgNS, 'title');
            title.textContent = `${d.formattedDate} • ${this.formatPrice(d.price)}`;
            c.appendChild(title);
            svg.appendChild(c);
            ...
            if (i % labelStep === 0 || i === data.length - 1) {
                ...
                tx.setAttribute('y', margin.top + H + 18);
                tx.setAttribute('text-anchor', 'middle');
                tx.setAttribute('class', 'x-label');
                tx.textContent = d.formattedDate;
                svg.appendChild(tx);
            }
        });

        wrapper.appendChild(svg);
        priceChart.appendChild(wrapper);

        // Legend
        const legend = document.createElement('div');
        legend.className = 'price-legend';
        legend.innerHTML = `
            <div class="legend-title">Price Range Legend</div>
        ...
        summary.innerHTML = `
            <div class="price-stats">
                <div class="stat-item">
                    ...
                    <span class="stat-value current">${this.formatPrice(currentPrice)}</span>
                </div>
                <div class="stat-item">
                    <span class="stat-label">${periodLabel} Low</span>
                    <span class="stat-value low">${this.formatPrice(minPrice)}</span>
                </div>
                <div class="stat-item">
                    <span class="stat-label">${periodLabel} High</span>
                    <span class="stat-value high">${this.formatPrice(maxPrice)}</span>
                </div>
            </div>
        `;
        priceChart.appendChild(summary);
    }
