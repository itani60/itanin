...
class PriceAlertModal {
    ...
    populateModal(product) {
        ...
        if (lowestPrice > 0) {
            targetPriceInput.value = Math.round(lowestPrice * 0.9);
        }
    }
    ...
    handlePriceAlertSubmission() {
        ...
        // Save the alert
        this.savePriceAlertToStorage(alertData);
        
        // Show success message
        this.showNotification(`Price alert set for ${alertData.productName}!`, 'success');
    ...
    savePriceAlertToStorage(alertData) {
        const alerts = JSON.parse(localStorage.getItem('priceAlerts') || '[]');
        
        // Remove existing alert for this product if it exists
        const filteredAlerts = alerts.filter(alert => alert.productId !== alertData.productId);
