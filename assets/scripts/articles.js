...
class ArticlesManager {
    ...
    renderArticles() {
        const articlesShowcase = document.querySelector('.latest-tech-news-reviews-showcase');
        if (!articlesShowcase) return;

        // Shuffle the articles array to show different ones on each refresh
        const shuffledArticles = this.shuffleArray([...this.articles]);
        
        // Show only 3 articles
        const articlesToShow = shuffledArticles.slice(0, 3);

        articlesShowcase.innerHTML = articlesToShow.map(article => `
            <div class="article-card" onclick="articlesManager.openArticleModal('${article.id}')">
